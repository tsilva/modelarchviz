...
class ResNet152(nn.Module):
    ...
    def __call__(self, x, train=False):
        ...
        x = self._stage(x, block, 128, blocks=8, stride=2, train=train)  # (batch, 56, 56, channels) -> (batch, 28, 28, channels)
        x = self._stage(x, block, 256, blocks=36, stride=2, train=train)  # (batch, 28, 28, channels) -> (batch, 14, 14, channels)
